...
class FormDumper {
  ...
  /**
   * Dump the content of a form as a HTML table formatted with the 'formdumper-table' CSS class. This class is not
   * available on the serviceportal instances, but might be included via the getCssClasses() method.
   *
  ...
  /**
   * Returns HTML code for including CSS that formats formdumper-table classes, as generated by the dumpFormAsHtmlTable
   * method. You should only use the output of this class once to avoid overriding the css class twice.
   */
  static String getCssClasses() {

    return """
      <style type="text/css">
        /* The table itself */
        table.formdumper-table {
          border-collapse: collapse; /* Make all cell share their borders (instead of having own borders) */
          width: 100%; /* Use the entire, available width */
          margin-bottom: 24px;
        }
    
        /* The cells */
        .formdumper-table th,.formdumper-table td {
        ...
        /* Highlight every second row (odd) */
        .formdumper-table tr:nth-child(odd) {
          background-color: var(--lt-color-background-default);
        }
    
        /* Highlight every second row */
        .formdumper-table tr:nth-child(even) {
          background-color: var(--lt-color-background-dark); 
        }
    
        /* Highlight hovered cells */
        .formdumper-table tr:hover {
          background-color: var(--call2action-contrast-color);
          color: var(--text-contrast-color);
        }
      </style>
    """.stripIndent()
