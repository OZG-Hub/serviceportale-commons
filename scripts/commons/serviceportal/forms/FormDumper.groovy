            ...
            if (shouldRenderField(field) && field.isShown(instance, formAndMapping)) {
              ...
              switch (field.type) {
                ...
                case FieldTypeV1.GDIK_MAP:
                  // Special handling for GDIK maü fields. Those fields are best represented as an image (rather than text)
                  BinaryGDIKMapContentV1 fieldContent = field.value as BinaryGDIKMapContentV1
                  int numberOfFiles = fieldContent.files.size()
                  assert numberOfFiles == 1: "Failed to render GDIK_MAP field '${field.id}' as it did contain " +
                          "${numberOfFiles} files instead of exactly one."
                  groupContentRendered += generateEmbeddedImage(fieldContent.files.first())
                  break
                default:
